...
export class FileController {
  ...
  async getFile(@Param("path") path: string, @Res() res: Response) {
    ...
    try {
      ...
      if (error instanceof ApplicativeError) {
        ...
        return res.status(500).json(new ApplicativeError(500, "Unknown error occurred"));
      }
    }
  }
  ...
  async renameFile(
    ...
    try {
      ...
      if (error instanceof ApplicativeError) {
        this.logger.error("Cannot renameFile", renameOperation, error.toString());
        return res.status(error.status).json(error);
      } else {
        this.logger.error("Cannot renameFile, unknown error", renameOperation);
        this.logger.debug("Cannot renameFile, unknown error", renameOperation, error);
        return res.status(500).json(new ApplicativeError(500, "Unknown error occurred"));
      }
    }
  }
